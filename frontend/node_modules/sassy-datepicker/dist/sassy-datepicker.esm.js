import React__default, { useMemo, memo, useCallback, createElement } from 'react';
...
var getDatesOfMonth = function getDatesOfMonth(date, minDateValue, maxDateValue, weekStartsFrom) {
  ...
  var currentMonth = date.getMonth(); // generate dates of each week of the month including the residue dates
  ...
  if (weekStartsFrom === 'Sunday') {
    ...
    for (var _i2 = 1; _i2 <= lastDayOfMonth.getDate(); _i2++) {
      var _d = new Date(date.getFullYear(), date.getMonth(), _i2);

      var dValue = _d.valueOf();
    ...
    for (var _i5 = 1; _i5 <= lastDayOfMonth.getDate(); _i5++) {
      var _d4 = new Date(date.getFullYear(), date.getMonth(), _i5);

      var _dValue = _d4.valueOf();

      dates.push({
        date: _d4,
        active: _dValue > minDateValue && _dValue < maxDateValue,
        ms: _dValue
      });
    }
...
function Option(_ref) {
  ...
  var handClick = React__default.useCallback(function () {
    if (!disabled) {
      onClick(value);
    }
...
var css_248z = ".sassy--select{border:none;border-radius:.25rem;display:inline;padding:.2rem .4rem}.sassy--select__container{border-radius:.25rem;position:relative}.sassy--select:focus{box-shadow:var(--ring-shadow);outline:none}.sassy--select__dropdown{background-color:#fff;border:none;border:var(--outline);border-radius:.5rem;box-shadow:var(--shadow-md);display:flex;flex-direction:column;max-height:40vh;overflow:auto;padding:.5rem;position:absolute;scrollbar-width:none;top:1.5rem;z-index:2}.sassy--select__dropdown::-webkit-scrollbar{display:none}.sassy--option,.sassy--select{font-family:var(--font);font-size:1rem;font-weight:500}.sassy--option:focus{box-shadow:var(--ring-shadow);outline:none}.sassy--option__disabled{color:var(--disabled-color);pointer-events:none}.sassy--select:hover{cursor:context-menu}.sassy--option{background-color:#fff;border:none;border-radius:.25rem;cursor:pointer;margin-top:.25rem;padding:.25rem .4rem;transition:all .1s ease}.sassy--option:first-of-type{margin-top:0}.sassy--option:hover,.sassy--option__active{background-color:var(--theme-color);box-shadow:var(--shadow);color:var(--selected-date-color)}";
...
function CustomSelect(_ref) {
  ...
  React__default.useEffect(function () {
    ...
    var blurListener = function blurListener(e) {
      ...
      if (!((_ref$current2 = ref.current) != null && _ref$current2.contains(e.target))) {
        closeOptionsDropdown();
      }
    };

    document.addEventListener('click', clickListener);
    (_ref$current3 = ref.current) == null ? void 0 : _ref$current3.addEventListener('focusout', blurListener);
    return function () {
      var _ref$current4;

      document.removeEventListener('click', clickListener);
      (_ref$current4 = ref.current) == null ? void 0 : _ref$current4.removeEventListener('focusout', blurListener);
    };
...
var Header = function Header(_ref) {
  ...
  var fromYear = React__default.useMemo(function () {
    return new Date(minDateValue).getFullYear();
  }, [minDateValue]);
  var toYear = React__default.useMemo(function () {
...
var DatePicker = /*#__PURE__*/React__default.forwardRef(function (_ref, ref) {
  ...
  var minDateValue = React__default.useMemo(function () {
    var _minDate$getTime;

    return (_minDate$getTime = minDate == null ? void 0 : minDate.getTime()) != null ? _minDate$getTime : new Date(1900, 0, 1).getTime();
  }, [minDate]);
  var maxDateValue = React__default.useMemo(function () {
    var _maxDate$getTime;

    return (_maxDate$getTime = maxDate == null ? void 0 : maxDate.getTime()) != null ? _maxDate$getTime : dt.addYears(new Date(), 100).getTime();
  }, [maxDate]); // current month and year the user is viewing

  var _React$useState = React__default.useState(value),
      openedDate = _React$useState[0],
      setOpenedDate = _React$useState[1];
  ...
  var onMonthChange = React__default.useCallback(function (month) {
    setOpenedDate(function (d) {
      return new Date(d.getFullYear(), getMonthNumberFromName(month), d.getDate());
    });
  }, [setOpenedDate]);
  var onYearChange = React__default.useCallback(function (year) {
    setOpenedDate(function (d) {
      return new Date(year, d.getMonth(), d.getDate());
    });
  }, [setOpenedDate]);
  var handleClick = React__default.useCallback(function (d) {
    return onChange == null ? void 0 : onChange(d);
  }, [onChange]);
  var daysOfWeekElements = React__default.useMemo(function () {
    return getDaysOfWeek(options.weekStartsFrom).map(function (v) {
  ...
  var daysOfMonthList = React__default.useMemo(function () {
    return getDatesOfMonth(openedDate, minDateValue, maxDateValue, options.weekStartsFrom);
  }, [openedDate, minDateValue, maxDateValue, options.weekStartsFrom]); // TODO: arrow-keys navigation
  ...
  }, React__default.createElement(React__default.Fragment, null, daysOfWeekElements), React__default.createElement(React__default.Fragment, null, daysOfMonthList.map(function (_ref2) {
    var date = _ref2.date,
        active = _ref2.active,
        ms = _ref2.ms;
...
var css_248z$2 = ".sdp,.stp{--font:inherit;--shadow:transparent 0px 0px 0px 0px,transparent 0px 0px 0px 0px,rgba(0,0,0,.1) 0px 1px 3px 0px,rgba(0,0,0,.06) 0px 1px 2px 0px;--shadow-md:transparent 0px 0px 0px 0px,transparent 0px 0px 0px 0px,rgba(0,0,0,.1) 0px 4px 6px -1px,rgba(0,0,0,.06) 0px 2px 4px -1px;--theme-color:#60a5fa;--selected-date-color:#fff;--disabled-color:#bdbdbd;--outline:1px solid #cbd5e1;--ring-shadow:0px 0px 0px 2px var(--theme-color)}";
styleInject(css_248z$2);

var css_248z$3 = ".stp{border:none;border:var(--outline);border-radius:.5rem;box-shadow:var(--shadow);display:inline-flex;font-family:var(--font);padding:.5rem .4rem}.stp,.stp *{outline:none}.stp:focus,.stp :focus{box-shadow:0 0 0 2px #60a5fab3}.stp>p{display:inline;margin:0 2px}";
styleInject(css_248z$3);

var _excluded$1 = ["onChange", "value", "minTime", "maxTime", "minutesInterval", "className"];
/**
 *
